    ...
    if request.method == 'POST':
        if request.form.get('action1') == 'Analizar':
            ...
            if file and allowed_file(file.filename):
                ...
                target_path = os.path.join('app', app.config['UPLOAD_FOLDER'], filename)
                file.save(target_path)

                seq_animal_kind, seq_confidence = identify_animal_kind(target_path, "sequence")
                vgg_animal_kind, vgg_confidence = identify_animal_kind(target_path, "vgg16")
                file_animal_tuples = [(filename, (seq_animal_kind, seq_confidence, vgg_animal_kind, vgg_confidence))]
